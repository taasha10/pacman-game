...
if(player.x <= powerdot.x && powerdot.x <= (player.x+32) && player.y <= powerdot.y && powerdot.y <= (player.y +32)){
	...
	enemy.ghostNum = 384;  enemy2.ghostNum = 384;
	powerdot.x =0;
	powerdot.y =0;
...
if(powerdot.ghosteat){
	...
	if (powerdot.pcountdown<=0) {
		...
		enemy.ghostNum = powerdot.ghostNum;   enemy2.ghostNum = powerdot.ghostNum;
		player.speed=8;
	}
	...
	if(powerdot.powerup){
		...
		context.beginPath();
		context.arc(powerdot.x,powerdot.y,8,0, Math.PI * 2,true);
		context.closePath();
		context.fill();
	}
